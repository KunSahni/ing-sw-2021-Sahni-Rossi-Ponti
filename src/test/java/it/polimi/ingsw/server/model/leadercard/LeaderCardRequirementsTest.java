...
class LeaderCardRequirementsTest {
    ...
    void getRequiredDevelopmentCardsTest(RepetitionInfo repetitionInfo) {
        if (repetitionInfo.getCurrentRepetition() == 0) {
            ...
            assertAll(
                    () -> assertEquals(actualLevel, Level.LEVEL2, "Error: LeaderCardRequirements returned a different level than the one passed"),
                    () -> assertEquals(1, actualQuantity, "Error: LeaderCardRequirements returned a different quantity than the one passed"),
                    () -> assertEquals(1, leaderCardRequirements.getRequiredDevelopmentCards().size(), "Error: LeaderCardRequirements contains more values than it should")
            );
            ...
            assertAll(
                    () -> assertEquals(2, blueQuantity, "Error: LeaderCardRequirements returned a different quantity for the first passed parameter"),
                    () -> assertEquals(1, yellowQuantity, "Error: LeaderCardRequirements returned a different quantity for the second passed parameter"),
                    () -> assertEquals(2, leaderCardRequirements.getRequiredDevelopmentCards().size(), "Error: LeaderCardRequirements contains more values than it should")
            );
        }else{
            Map<Color, LeaderCardRequirements.LevelQuantityPair> developmentCardsRequirement = new HashMap<>();
            ...
            assertAll(
                    () -> assertEquals(1, blueQuantity, "Error: LeaderCardRequirements returned a different quantity for the first passed parameter"),
                    () -> assertEquals(1, yellowQuantity, "Error: LeaderCardRequirements returned a different quantity for the second passed parameter"),
                    () -> assertEquals(2, leaderCardRequirements.getRequiredDevelopmentCards().size(), "Error: LeaderCardRequirements contains more values than it should")
            );
            }
    ...
    void getRequiredResourcesTest() {
        HashMap<Resource, Integer> expectedRequiredResources = new HashMap<>();
    ...
    void equalsTest() {
        ...
        expectedRequiredResources.put(Resource.COIN, 5);
        leaderCardRequirements = new LeaderCardRequirements(null, expectedRequiredResources);
        LeaderCardRequirements leaderCardRequirements2 = new LeaderCardRequirements(null, expectedRequiredResources);
        LeaderCardRequirements leaderCardRequirements3 = new LeaderCardRequirements(null, null);
        assertAll(
                () ->  assertEquals(leaderCardRequirements, leaderCardRequirements2, "Error: equals method returned false on two objects which are the same"),
                () ->assertNotEquals(leaderCardRequirements, leaderCardRequirements3, "Error: equals method returned true on two objects which are not the same")
        );
